...
import (
	...
	"github.com/sniperkit/gargantua/lib/sitemap"
)
...
func getURLs(xmlSitemapURL url.URL) ([]url.URL, error) {
	...
	urlsFromIndex, indexError := sitemap.GetURLsFromSitemapIndex(xmlSitemapURL)
	if indexError == nil {
		urls = urlsFromIndex
	}

	urlsFromSitemap, sitemapError := sitemap.GetURLsFromSitemap(xmlSitemapURL)
	if sitemapError == nil {
