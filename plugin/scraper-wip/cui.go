...
import (
	...
	cfg "github.com/asciimoo/wuzz/config"
...
func (a *App) LoadConfig(configPath string) error {
	if configPath == "" {
		...
		configPath = cfg.GetDefaultConfigLocation()
	}

	// If the config file doesn't exist, load the default config
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		a.config = &cfg.DefaultConfig
		a.config.Keys = cfg.DefaultKeys
...
func (a *App) ParseArgs(g *gocui.Gui, args []string) error {
	...
	for arg_index < args_len {
		...
		case "-j", "--json":
			...
			content_type = "json"
			accept_types = append(accept_types, cfg.ContentTypes["json"])
			set_data = true
		...
		case "-t", "--timeout":
			...
			a.config.General.Timeout = cfg.Duration{Duration: time.Duration(timeout) * time.Millisecond}
		case "--compressed":
